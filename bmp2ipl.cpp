...
*    the IplImage use TopLeft （頂左）                                                            *
*  IplImage:                                                                                      *
*    nChannels = 1 or 3       number of channels                                                  *
*    depth = IPL_DEPTH_8U     pixel depth (8 bit), IPL_DEPTH_8U=8                                 *
*    dataOrder = 0            交叉存取顏色通道                                                    *
*    origin = IPL_ORIGIN_TL   圖像數據保存形式，IPL_ORIGIN_BL底左結構, IPL_ORIGIN_TL頂左結構      *
*    align = 4                行數據對齊方式，保證下一行數據從整4位元組位置開始                     *
*    width                                                                                        *
*    height                                                                                       *
*    widthStep                圖像數據行大小，單位位元組                                            *
*    imageSize                圖像數據大小(=height*widthStep)，單位位元組                           *
*    imageData                指向圖像數據區，char *                                              *
*  BITMAPINFOHEADER:                                                                              * 
*    biSize                   Specifies the number of bytes required by the structure             *
*    biWidth                                                                                      *
*    biHeight(>0)             biHeight>0底左結構；biHeight<0頂左結構                              *
...
{   
	...
	CopyData((char *)bmpData, (char*)img.bmpData, biHeader.biSizeImage,!isLowerLeft, biHeader.biHeight);
}
...
BMP::BMP(const IplImage &img) 
{
	...
	CopyData((char*)bmpData, (char*)img.imageData, img.imageSize,isTopLeft, img.height);
		/*int i,j;
		CvScalar s;
...
void BMP::CopyData(char *dest, const char *src, int dataByteSize,bool isConvert, int height)
 {
	char * p = dest;
	if(!isConvert) 
	{   
...
IplImage * BMP::BMP2Ipl() 
{
	...
	CopyData( iplImg->imageData, (char*)bmpData, biHeader.biSizeImage,isLowerLeft, height);
		/*int i,j;
		CvScalar s;
